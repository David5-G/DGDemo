...
- (void)setupUI {
    ...
    UIImage *backImage = [UIImage imageNamed:@"navi_back_gray"];
...
-(void)setBarType:(DGNavigationBarType)barType {
    ...
    switch (barType) {
        case DGNavigationBarTypeDefault:{
            self.bgColor = KKES_COLOR_NAVI_BG;
        ...
        case DGNavigationBarTypeGray:{
            self.bgColor = KKES_COLOR_HEX(0xf2f2f2);
            self.titleLabel.textColor = KKES_COLOR_HEX(0x333333);
            [self.backButton setImage:backImgGray forState:UIControlStateNormal];
        }break;
