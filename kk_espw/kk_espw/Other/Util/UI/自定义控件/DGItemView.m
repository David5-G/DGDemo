...
-(void)clickItemButton:(DGItemButton *)sender {
    ...
    BOOL effective = YES;
    if (self.delegate && [self.delegate respondsToSelector:@selector(itemView:didSelectedAtIndex:)]) {
        effective = [self.delegate itemView:self didSelectedAtIndex:_selectedIndex];
    }
    if(!effective){
        return ;
    }
...
- (void)updateIndicatorViewPosition {
    ...
    if (self.indicatorStyle == DGItemViewIndicatorStyleLine) {
        indicatorCenter = CGPointMake(btn.center.x, btnH - self.lineIndicatorHeight/2.0 - self.lineIndicatorBottomSpace/2.0);
        indicatorBounds = CGRectMake(0, 0, btnW * self.indicatorScale,self.lineIndicatorHeight);
        cornerRadius = self.lineIndicatorCornerRadius;
    }
...
-(void)setIndicatorScale:(CGFloat)indicatorScale {
    ...
    if (self.indicatorStyle == DGItemViewIndicatorStyleLayer) {
        if (self.needEqualWidth) {
            indicatorScale = indicatorScale >= 0.1 ? indicatorScale : 0.1;
        }else{
            indicatorScale = 1.0;
        }
    }
...
-(void)setLineIndicatorBottomSpace:(CGFloat)lineIndicatorBottomSpace {
    //1.限制最大值
    if (lineIndicatorBottomSpace > self.bounds.size.height) {
        lineIndicatorBottomSpace = self.bounds.size.height;
    }
    
    //2.限制最小值
    if (lineIndicatorBottomSpace < 0) {
        lineIndicatorBottomSpace = 0;
    }
    
    //3.赋值
    _lineIndicatorBottomSpace = lineIndicatorBottomSpace;
}
...
-(void)setLineIndicatorCornerRadius:(CGFloat)lineIndicatorCornerRadius {
    //1.限制最大值
    if (lineIndicatorCornerRadius > self.lineIndicatorHeight/2.0) {
        lineIndicatorCornerRadius = self.lineIndicatorHeight/2.0;
    }
    //2.限制最小值
    if (lineIndicatorCornerRadius < 0) {
        lineIndicatorCornerRadius = 0;
    }
    
    //3.赋值
    _lineIndicatorCornerRadius = lineIndicatorCornerRadius;
}
