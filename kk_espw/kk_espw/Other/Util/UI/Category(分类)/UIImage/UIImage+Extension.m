...
- (UIImage*)imageRotatedByAngle:(CGFloat)Angle
{
    ...
    CGContextRotateCTM(context, Angle * M_PI / 180); //* M_PI / 180
    CGContextRotateCTM(context, M_PI);
    CGContextScaleCTM(context, -1.0, 1.0);
    CGContextDrawImage(context, CGRectMake(-rotatedSize.width/2, -rotatedSize.height/2, rotatedSize.width, rotatedSize.height), self.CGImage);
    UIImage* newImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    //4.return
    return newImage;
...
{
    return [self imageWithColor:color size:CGSizeMake(10.0f, 10.0f)];
}
...
- (UIImage *)compressToMaxKbSize:(CGFloat)KbSize{
    ...
    while (dataKBytes > KbSize && maxQuality > 0.01f) {
