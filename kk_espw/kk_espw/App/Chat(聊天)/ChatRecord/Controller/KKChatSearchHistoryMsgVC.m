...
-(void)setupNavi {
    ...
    //3.cancelBtn
    DGButton *cancelBtn = [DGButton btnWithFontSize:[ccui getRH:18] title:@"取消" titleColor:[UIColor grayColor]];
...
-(void)setupUI {
    ...
    [tableV setSeparatorColor:[UIColor whiteColor]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (![self.targetId isEqualToString:userId]) {
         name = msg.content.senderUserInfo.name;
         head = msg.content.senderUserInfo.portraitUri;
    } else {
        NSString *extra = msg.content.senderUserInfo.extra;
        if (extra) {
            NSData *jsonData = [extra dataUsingEncoding:NSUTF8StringEncoding];
            NSError *error;
            NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingMutableContainers error:&error];
            if (!error) {
                name = [dic objectForKey:@"conversationTitle"];
                head = [dic objectForKey:@"head"];
            }
        }
    }

    [cell.headIconImageView sd_setImageWithURL:[NSURL URLWithString:head]];
...
-(void)pushToChatVC:(RCMessage *)msg {
    KKConversationController *conversationController = [[KKConversationController alloc] initWithConversation:_conversation extra:msg];
