...
@implementation KKHomeRecommendCell
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        whiteBgV.frame = CGRectMake(RH(10), RH(10), SCREEN_WIDTH - RH(10) * 2, RH(143));
...
- (void)setIsSearch:(BOOL)isSearch {
    if (isSearch == YES) {
        _roomIDLabel.textColor = KKES_COLOR_MAIN_YELLOW;
    }else {
        _roomIDLabel.textColor = KKES_COLOR_GRAY_TEXT;
    }
}
...
- (void)setInfo:(KKHomeRoomListInfo *)info {
    ...
    if ([info.sex.name isEqualToString:@"F"]) {
        ...
        _sexImageView.size = CGSizeMake(RH(7), RH(11));
    }else {
