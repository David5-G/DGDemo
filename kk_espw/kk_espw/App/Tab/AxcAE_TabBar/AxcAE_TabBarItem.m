...
- (void)itemDidLayoutBadgeLabel{
    switch (self.itemModel.itemBadgeStyle) {
        case AxcAE_TabBarItemBadgeStyleTopRight:{   // 右上方 默认
            CGFloat width = (self.backgroundImageView.size.width -  self.icomImgView.image.size.width)/ 2;
            self.badgeLabel.center = CGPointMake(self.icomImgView.image.size.width + width + 5 ,self.badgeLabel.frame.size.height/2 );
        }break;
