...
#import "KKLiveStudioUserMgrPopVC.h"
//view
#import "DGItemView.h"
#import "KKLiveStudioMicRankListView.h"
#import "KKLiveStudioOnlineUserListView.h"
#import "KKLiveStudioForbidWordUserListView.h"
#import "KKLiveStudioMicRankCell.h"
#import "KKLiveStudioOnlineUserCell.h"
#import "KKLiveStudioForbidWordUserCell.h"

@interface KKLiveStudioUserMgrPopVC ()
<UIScrollViewDelegate,
DGItemViewDelegate,
KKLiveStudioUserMgrDelegate>
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) DGItemView *itemView;
@property (nonatomic, strong) NSArray <NSString *>*titleArray;

//排麦
@property (nonatomic, strong) KKLiveStudioMicRankListView *micListView;
//在线
@property (nonatomic, strong) KKLiveStudioOnlineUserListView *onlineListView;
//禁言
@property (nonatomic, strong) KKLiveStudioForbidWordUserListView *forbidWordListView;
@end

@implementation KKLiveStudioUserMgrPopVC
#pragma mark - init
-(instancetype)init {
...
-(void)scrollViewAddMicRankListView:(CGRect)frame {
    ...
    WS(weakSelf);
    //1.排麦用户UI
    KKLiveStudioMicRankListView *mListV = [[KKLiveStudioMicRankListView alloc]initWithFrame:frame];
    mListV.isHostCheck = YES;
    [mListV setConfigBlock:^(KKLiveStudioMicRankCell *cell, KKLiveStudioUserSimpleModel *model, NSIndexPath *indexPath) {
    ...
    } selectBlock:^(KKLiveStudioUserSimpleModel *model, NSIndexPath *indexPath) {
...
-(void)scrollViewAddOnlineUserListView:(CGRect)frame {
     ...
     WS(weakSelf);
    //1.在线用户UI
    KKLiveStudioOnlineUserListView *oListV = [[KKLiveStudioOnlineUserListView alloc]initWithFrame:frame];
    [oListV setConfigBlock:^(KKLiveStudioOnlineUserCell *cell, KKLiveStudioUserSimpleModel *model, NSIndexPath *indexPath) {
        cell.nameLabel.text = model.loginName;
        [cell.portraitImageView sd_setImageWithURL:Url(model.userLogoUrl)];
        cell.isHostCheck = YES;
        cell.isOnMic = model.onMic;
...
-(void)scrollViewAddForbidWordListView:(CGRect)frame {
    ...
    WS(weakSelf);
    //1.禁言用户UI
    KKLiveStudioForbidWordUserListView *fListV = [[KKLiveStudioForbidWordUserListView alloc]initWithFrame:frame];
    fListV.isHostCheck = YES;
    [fListV setConfigBlock:^(KKLiveStudioForbidWordUserCell *cell, KKLiveStudioUserSimpleModel *model, NSIndexPath *indexPath) {
        cell.nameLabel.text = model.loginName;
        [cell.portraitImageView sd_setImageWithURL:Url(model.userLogoUrl)];
        cell.isHostCheck = YES;
        cell.delegate = weakSelf;
...
#pragma mark tool
-(void)showPlayerGameCard:(KKLiveStudioUserSimpleModel *)model {
     [[KKLiveStudioPlayerGameCardTool shareInstance] showUserInfo:model.userId onMic:model.onMic needReport:NO isHostCheck:YES isHorizatalButton:YES delegate:self];
}
...
#pragma mark KKLiveStudioUserMgrDelegate
-(void)triggerView:(UIView *)view setMicGive:(BOOL)isGive {
    
    //1.是MicRankCell
    if ([view isMemberOfClass:[KKLiveStudioMicRankCell class]]) {
        KKLiveStudioMicRankCell *cell = (KKLiveStudioMicRankCell *)view;
        ...
        //2.请求给麦
        KKLiveStudioUserSimpleModel *model = self.userListViewModel.micRankUsers[row];
        [self.studioViewModel requestMicGive:model.userId userIsRanking:YES success:^(NSInteger code) {
            if (code >= 0) {
                cell.isOnMic = YES;
            }
        }];
    ...
    //2.是onlineUserCell
    if ([view isMemberOfClass:[KKLiveStudioOnlineUserCell class]]) {
        KKLiveStudioOnlineUserCell *cell = (KKLiveStudioOnlineUserCell *)view;
        ...
        //2.请求给麦
        KKLiveStudioUserSimpleModel *model = self.userListViewModel.onlineUsers[row];
        [self.studioViewModel requestMicGive:model.userId userIsRanking:NO success:^(NSInteger code) {
            if (code >= 0) {
                cell.isOnMic = YES;
            }
        }];
...
-(void)triggerView:(UIView *)view setForbidWord:(BOOL)isForbid {
    //1.禁言
    if ([view isMemberOfClass:[KKLiveStudioOnlineUserCell class]] && isForbid) {
        [self requestForbidWord:(KKLiveStudioOnlineUserCell *)view];
        return ;
    }
    
    //2.取消禁言
    if ([view isMemberOfClass:[KKLiveStudioForbidWordUserCell class]] && !isForbid) {
        [self requestForbidWordCancel:(KKLiveStudioForbidWordUserCell *)view];
        return ;
    }
}

#pragma mark KKLiveStudioPlayerGameCardToolDelagate
-(void)playerGameCardTool:(KKLiveStudioPlayerGameCardTool *)tool handleMsg:(NSString *)msg {
    //1.下麦
    if ([msg isEqualToString:@"下麦"] ||
        [msg isEqualToString:@"抱下麦"]) {
        [self requestMicQuit:tool.userId];
        return;
    }
}

#pragma mark - request
#pragma mark mic quit
/** 请求 下麦 */
-(void)requestMicQuit:(NSString *)userId {
    WS(weakSelf);
    [self.studioViewModel requestMicQuit:userId success:^(NSInteger code) {
        if (code >= 0) {
            [weakSelf refreshListViewAtIndex:weakSelf.itemView.selectedIndex];
        }
    }];
}
...
-(void)requestForbidWord:(KKLiveStudioOnlineUserCell *)cell {
    
    //1.不是在线cell
    if (![cell isMemberOfClass:[KKLiveStudioOnlineUserCell class]]) {
        return ;
    }
...
-(void)requestForbidWordCancel:(KKLiveStudioForbidWordUserCell *)cell {
    //1.不是在线cell
    if (![cell isMemberOfClass:[KKLiveStudioForbidWordUserCell class]]) {
        return;
    }
    ...
    //3.获取model
    KKLiveStudioUserSimpleModel *model = self.userListViewModel.forbidWordUsers[row];
